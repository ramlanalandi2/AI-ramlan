import { fetchMessagesFromApi, exportApiData, sendMessageViaApi, sendToThreadViaApi, fetchMessageTemplates, sendTemplateViaApi, sendStructuredMessage } from './graph-service.js';
...
Usage:
  ...
  node index.js api-template-send <recipient_id> <access_token> <template_name> [language]
  node index.js api-button <recipient_id> <access_token> <text> <button_title> <url>
  node index.js api-card <recipient_id> <access_token> <title> <subtitle> <button_title> <url>
...
const main = async () => {
  ...
  try {
    if (command === 'export') {
    ...
    else if (command === 'api-template-send') {
      let recipientId = args[1];
      let accessToken = args[2];
      let templateName = args[3];
      let language = args[4] || 'id_ID';

      if (!recipientId || !accessToken || !templateName) {
        log.error('Recipient ID, Access Token, and Template Name are required.');
        return;
      }

      await sendTemplateViaApi(recipientId, accessToken, templateName, language);
    }
    else if (command === 'api-button') {
      const [recipientId, accessToken, text, btnTitle, url] = args.slice(1);
      if (!recipientId || !accessToken || !text || !btnTitle || !url) {
        log.error('Missing arguments for api-button');
        return;
      }
      const payload = {
        text: text,
        buttons: [{ type: 'web_url', url: url, title: btnTitle }]
      };
      await sendStructuredMessage(recipientId, accessToken, 'button', payload);
    }
    else if (command === 'api-card') {
      const [recipientId, accessToken, title, subtitle, btnTitle, url] = args.slice(1);
      if (!recipientId || !accessToken || !title || !subtitle || !btnTitle || !url) {
        log.error('Missing arguments for api-card');
        return;
      }
      const payload = {
        elements: [{
      ...
      await sendStructuredMessage(recipientId, accessToken, 'generic', payload);
    }
